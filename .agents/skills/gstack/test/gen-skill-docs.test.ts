...
describe('setup script validation', () => {
  ...
  test('auto mode detects claude, codex, kiro, and cursor binaries', () => {
  ...
  test('setup supports --host cursor with install section and generated skills', () => {
    expect(setupContent).toContain('INSTALL_CURSOR=');
    expect(setupContent).toContain('CURSOR_SKILLS=');
    expect(setupContent).toContain('~/.cursor/skills/gstack');
    expect(setupContent).toContain('create_cursor_runtime_root');
    expect(setupContent).toContain('link_cursor_skill_dirs');
    expect(setupContent).toContain('gen:skill-docs --host cursor');
  });

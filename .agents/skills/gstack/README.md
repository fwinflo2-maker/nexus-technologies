...
One command from the gstack repo (or after a global install):

```bash
bin/project-integrate --host auto --dir /path/to/your/project
```

This vendors gstack into the project, runs setup, and adds a gstack section to `AGENTS.md` (Cursor Cloud / Codex / Gemini) and `CLAUDE.md` (Claude Code, when installed). Commit the vendored directory so `git clone` just works.

Or paste this to Claude Code for manual integration:

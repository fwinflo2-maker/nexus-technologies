...
final class ControlCenterController
{
    ...
    /**
     * Met à jour le statut client et invalide les JWT en cours lors d'une
     * restriction. Si la migration `password_changed_at` n'est pas appliquée,
     * on retombe sur la mise à jour du statut seul (AuthMiddleware bloque quand
     * même l'accès au prochain appel).
     */
    private static function applyClientStatusChange(\PDO $pdo, int $id, string $status): void
    {
        if (in_array($status, ['SUSPENDED', 'CLOSED'], true)) {
            try {
                $pdo->prepare(
                    'UPDATE users SET status = :status, password_changed_at = UTC_TIMESTAMP() WHERE id = :id'
                )->execute(['status' => $status, 'id' => $id]);

                return;
            } catch (\PDOException) {
                // Migration absente : statut seul, sans invalidation JWT historique.
            }
        }

...
final class AdminController
{
    ...
    public static function inviteEmployee(Request $request): void
    {
        ...
        $resetPath = '/forgot-password?token=' . $token . '&portal=staff';
        Mailer::sendPasswordReset(
            (string) $row['email'],
            '',
            Mailer::frontendBaseUrl() . $resetPath,
            (int) (self::EMPLOYEE_INVITE_TTL / 60)
        );
        Response::success([
            'employee_id' => $id,
            'expires_in' => self::EMPLOYEE_INVITE_TTL,
            'reset_token' => $isDevelopment ? $token : null,
            'reset_url' => $isDevelopment ? $resetPath : null,
            'delivery' => $isDevelopment ? 'DEVELOPMENT_MANUAL_DELIVERY' : 'EMAIL_SENT',
        ]);
    }

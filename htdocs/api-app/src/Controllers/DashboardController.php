...
final class DashboardController
{
    ...
    public static function summary(Request $request): void
    {
        ...
        // Si le bandeau KYC serait encore affiché, relire Sumsub (webhook souvent
        // absent en sandbox) puis recharger le user pour banner / limits.
        if (
            (($user['status'] ?? '') === 'PENDING' || !PolicyEngine::isVerified($user))
            && (new SumsubAdapter())->isConfigured()
        ) {
            $subject = (($user['account_type'] ?? '') === 'business')
                ? KycSubjectType::COMPANY
                : KycSubjectType::INDIVIDUAL;
            try {
                KycService::syncFromProvider($pdo, new SumsubAdapter(), $userId, $subject);
                $reload = $pdo->prepare(
                    'SELECT id, full_name, email, phone, account_type, platform_role, status,
                            kyc_level, kyb_status, avatar, country_of_residence, birth_date
                ...
                // Ne jamais casser le dashboard si Sumsub est down.
            }
        }
    ...
    private static function banner(array $user, float $totalAvailableRef): array
    {
        ...
        // 1. Vérification d'identité requise — uniquement si le compte n'est
        //    pas déjà vérifié (KYC standard+/KYB). Sinon le bandeau resterait
        //    collé même après un webhook Sumsub GREEN (status PENDING oublié).
        if ($status === 'PENDING' && !PolicyEngine::isVerified($user)) {
            return [
                'type'    => 'kyc',

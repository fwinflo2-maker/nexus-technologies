...
final class SumsubAdapter implements KycProvider
{
    ...
    {
        $this->hydrateFromPlatformIfNeeded();
        $managedKey = $type->isCompany() ? 'level_name_kyb' : 'level_name';
        if ($this->managedCredentials !== null) {
            $fromDb = trim((string) ($this->managedCredentials[$managedKey] ?? ''));
            if ($fromDb !== '') {
                return $fromDb;
            }
            // SuperAdmin : clés collées sans nom de niveau → niveau sandbox
            // documenté. Le KYB n'a pas de défaut (niveau entreprise dédié).
            if (!$type->isCompany()) {
    ...
    /**
     * Credentials SuperAdmin (base chiffrée) d'abord, .env ensuite.
     * `fromCredentialManager()` pose un tableau (éventuellement vide) et
     * désactive ce repli — les tests d'isolation restent honnêtes.
     */
    private function hydrateFromPlatformIfNeeded(): void
    {
        if ($this->managedCredentials !== null) {
            return;
        }
        try {
            ...
            $creds = \Nexus\Services\ProviderCredentialService::resolvePlatform(
            ...
            $token  = trim((string) ($creds['app_token'] ?? ''));
            $secret = trim((string) ($creds['secret_key'] ?? ''));
            if ($token === '' || $secret === '') {
                return;
            }
            $this->managedCredentials = $creds;
            $this->managedEnvironment = $env;
        } catch (\Throwable) {
            // Base illisible → on reste sur le bootstrap .env.
        }
    }
    ...
    public function testConnection(string $environment, ?array $credentials = null): array
    {
        if (is_array($credentials) && $credentials !== []) {
            ...
            $this->managedEnvironment = $environment === 'production' ? 'production' : 'sandbox';
        }

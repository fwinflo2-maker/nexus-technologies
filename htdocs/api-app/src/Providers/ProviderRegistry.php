...
final class ProviderRegistry
{
    ...
    {
        if (self::hasExactPlatformCredentials($slug, $environment)) {
            return true;
        }
        // Stripe Issuing utilise la Secret Key Stripe du Super Admin
        // (même compte, même environnement) si aucune clé dédiée n'existe.
        if ($slug === 'stripe_issuing') {
            return self::hasExactPlatformCredentials('stripe', $environment);
        }
        return false;
    }

    private static function hasExactPlatformCredentials(string $slug, string $environment): bool
    {
        try {

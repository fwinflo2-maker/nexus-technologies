...
final class ControlCenterService
{
    ...
    public static function providerCard(PDO $pdo, int $userId, string $slug): array
    {
        ...
        foreach (['sandbox', 'production'] as $env) {
            ...
            // Cartes virtuelles : même Secret Key Stripe que le Super Admin.
            if ($slug === 'stripe_issuing' && !$environments[$env]['configured']) {
                $stripeRow = ProviderCredentialService::findEffectiveRow($pdo, $userId, 'stripe', $env);
                if ($stripeRow !== null && ($stripeRow['credentials_enc'] ?? null) !== null) {
                    $environments[$env]['configured'] = true;
                    $environments[$env]['inherited_from'] = 'stripe';
                    $environments[$env]['status'] = (string) ($stripeRow['status'] ?? 'inherited');
                    $environments[$env]['last_tested_at'] = $stripeRow['last_tested_at'] ?? null;
                    $environments[$env]['last_error'] = $stripeRow['last_error'] ?? null;
                    $environments[$env]['updated_at'] = $stripeRow['updated_at'] ?? null;
                    $environments[$env]['base_url'] = ProviderConfig::baseUrl('stripe', $env);
                }
            }
        }

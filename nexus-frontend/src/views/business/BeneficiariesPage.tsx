  ...
  return (
    <div className="page">
      <motion.div className="page-header animate-up" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="page-label">BÉNÉFICIAIRES</div>
        <div className="page-title">Bénéficiaires</div>
      ...
      <div className="card" style={{ padding: 20, marginBottom: 20 }}>
        <div className="page-label" style={{ marginBottom: 14 }}>Nouveau bénéficiaire</div>
        <form onSubmit={create} style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          <button className="se-cta" type="submit" disabled={saving} style={{ fontSize: 12 }}>{saving ? 'Création…' : '+ Créer'}</button>
        </form>
      ...
      {loading ? (
        ...
        <div className="card card-hi-c" style={{ padding: 40, textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-mid)' }}>No beneficiaries yet — créez votre premier bénéficiaire ci-dessus.</p>
        </div>
      ) : (
            ...
            <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: 14, borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {b.verification_status !== 'verified' && (
                  <button className="pill p-c" style={{ cursor: 'pointer', fontSize: 11 }} onClick={() => verify(b.id)}>Vérifier</button>
                )}

...
final class AccountController
{
    ...
    public static function seedDemoAccountsAtLogin(\PDO $pdo, int $userId): void
    {
        ...
        $demo = [
            [
                'role' => 'source', 'kind' => 'bank_iban',
                'label' => 'Compte courant — Swan',
                'holder_name' => 'Florenzo Demo',
                'country' => 'FR', 'currency' => 'EUR',
                'iban_enc' => Crypto::encrypt('FR76 3000 4000 0123 4567 8910 123'),
                'bic' => 'SwanFRPP',
            ...
            [
                'role' => 'source', 'kind' => 'crypto_wallet',
                'label' => 'Wallet USDT (Ethereum)',
                'country' => 'FR', 'currency' => 'USDT',
                'network' => 'Ethereum',
                'address_enc' => Crypto::encrypt('0x1234567890abcdef1234567890abcdef12345678'),
                'verification_status' => 'verified',
                'supported_for_transfer' => 1,
                'status' => 'active',
            ],
        ...
        $ins = $pdo->prepare(
            'INSERT INTO payment_accounts
                (user_id, role, kind, label, holder_name, country, currency, iban_enc, bic, network, address_enc, is_default, verification_status, supported_for_transfer, status, created_at)
             VALUES
                (:user_id, :role, :kind, :label, :holder_name, :country, :currency, :iban_enc, :bic, :network, :address_enc, :is_default, :verification_status, :supported_for_transfer, :status, :created_at)'
        );

        $createdAt = gmdate('Y-m-d H:i:s', time() - 86400);
        ...
        foreach ($demo as $i => $row) {
            $ins->execute([
                ...
                'iban_enc'               => $row['iban_enc'] ?? null,
                'bic'                    => $row['bic'] ?? null,
                'network'                => $row['network'] ?? null,
                'address_enc'            => $row['address_enc'] ?? null,
                'is_default'             => $i === 0 ? 1 : 0,
                'verification_status'    => $row['verification_status'] ?? 'unverified',
                'supported_for_transfer' => $row['supported_for_transfer'] ?? 0,

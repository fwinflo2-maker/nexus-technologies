...
/**
 * CountryCodes — conversion ISO 3166-1 alpha-2 → alpha-3.
 *
 * Nexus stocke la résidence en alpha-2 (`users.country_of_residence`, char(2))
 * conformément au formulaire d'inscription. L'API Sumsub, elle, attend le pays
 * d'une entreprise en alpha-3 (`companyInfo.country`, ex. GBR, FRA, DEU) — voir
 * https://docs.sumsub.com/reference/autokyb-api-examples.
 *
 * Cette table fait le pont entre les deux vocabulaires. Elle ne contient aucun
 * secret : c'est une donnée publique normalisée ISO 3166-1.
 */
final class CountryCodes
{
    /** @var array<string,string> alpha-2 (majuscule) → alpha-3 */
    ...
    /**
     * Convertit un code alpha-2 en alpha-3.
     *
     * Retourne null si le code est vide ou inconnu : l'appelant ne doit JAMAIS
     * transmettre un code deviné à un provider — une valeur inconnue est omise
     * plutôt que falsifiée (§37 : ne jamais simuler une donnée de conformité).
     */
    public static function alpha2ToAlpha3(?string $alpha2): ?string
    {
        $code = strtoupper(trim((string) $alpha2));
        if ($code === '') {
            return null;
        }
        return self::ALPHA2_TO_ALPHA3[$code] ?? null;
    }
}
